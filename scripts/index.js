...
const profileFormInputs = Array.from(formEdit.querySelectorAll('.form__input'));
const AddFormInputs = Array.from(formCard.querySelectorAll('.form__input'));
const btnEditSave = formEdit.querySelector('.popup__save-btn');
const btnCreateSave = formCard.querySelector('.popup__save-btn');
...
buttonEditOpen.addEventListener('click', function () {
  ...
  profileFormInputs.forEach((inputElement) => {
    checkInputValidity(formEdit, inputElement, valid);
  });
});
